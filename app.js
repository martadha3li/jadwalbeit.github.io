...
window.addEventListener("DOMContentLoaded", async ()=>{
  setTimeout(()=>{ document.getElementById("splashScreen").style.display="none"; document.getElementById("loginPage").style.display="block"; },500);
  await fetchMembers();
});
...
function loadSchedules(){
  const washHTML = washingMembers.map(m=>`🔹 ${m} يغسل هذا الأسبوع`).join("<br>");
  const cleanHTML = cleaningMembers.map(m=>`🧹 ${m} ينظف هذا الأسبوع`).join("<br>");
  const cookHTML = showCooking ? cookingMembers.map(m=>`🍳 ${m} يطبخ هذا الأسبوع`).join("<br>") : "تم إخفاء جدول الطبخ";
  const tamweenHTML = tamweenMembers.map(m=>`📦 ${m} تموين`).join("<br>");
...
// Toggle cooking
function toggleCooking(){ showCooking=!showCooking; loadSchedules(); }
...
async function login(){
  ...
  if(userDoc.exists()){
    ...
    if(data.password===pass){
      ...
      loadSchedules(); renderUsers(); renderFees();
...
window.renderFees=renderFees; window.payNow=payNow; window.toggleCooking=toggleCooking;
